...
function applyPromo(promoCode, type, basket) {
  ...
  if (p.type === 'percent') {
    ...
    // Бесплатная доставка — цены товаров не меняются, только скидка доставки (которой у нас нет как отдельной статьи)
    discount = 0;
...
async function handleDeliveryDM(event) {
  ...
  if (text === 'начать' || text === '/start') {
    ...
    sess.data = {};
    await sendMessage(peerId, 'Добро пожаловать! Выберите раздел:', { keyboard: DELIVERY_MAIN_KB }, 2);
    return;
  }

  if (text === 'Главное меню') {
    // Запрещаем выход в меню пока заказ активен (в ожидании или обрабатывается)
    if (sess.step === DEL_STEP.WAITING || sess.step === DEL_STEP.ACTIVE) {
      const activeOrder = storage.activeOrders.get(sess.data.orderId);
      if (activeOrder && ['accepted', 'delivering', 'arrived'].includes(activeOrder.status)) {
        await sendMessage(peerId, 'Нельзя выйти в меню пока заказ в обработке. Дождитесь завершения заказа или отмените его.', {}, 2);
        return;
      }
      // Заказ ещё pending или не найден — можно выйти
      sess.step = DEL_STEP.MAIN;
      sess.data = {};
      await sendMessage(peerId, 'Главное меню. Выберите раздел:', { keyboard: DELIVERY_MAIN_KB }, 2);
      return;
    }
    sess.step = DEL_STEP.MAIN;
    sess.data = {};
  ...
  if (sess.step === DEL_STEP.BASKET) {
    ...
    if (text === 'Оформить заказ') {
      ...
      // Сначала промокод, потом ник и адрес
      sess.step = DEL_STEP.PROMO;
      sess.data.promo = null;
  ...
  if (sess.step === DEL_STEP.CHECKOUT_ADDR) {
    ...
    const promoResult = sess.data.promo || null;
    const costTotalPreview = sess.data.basket.reduce((sum, it) => sum + (it.cost || 0) * it.qty, 0);
    const rawTotal = basketTotal(sess.data.basket);
    let previewTotal = rawTotal - (promoResult?.discount || 0);
    if (promoResult?.promo?.type === 'free_delivery') previewTotal = costTotalPreview;
    const promoLine = promoResult ? `\nПромокод: ${promoResult.msg}` : '';
    const summary = `Проверьте данные заказа:\n\nНикнейм: ${sess.data.nick}\nМесто: ${sess.data.address}\n\n${buildBasketText(sess.data.basket)}${promoLine}\nИтого к оплате: ${previewTotal}р.\n\nВсё верно?`;
    await sendMessage(peerId, summary, { keyboard: msgKb([[{ label: 'Верно', color: 'positive' }, { label: 'Изменить', color: 'secondary' }]]) }, 2);
    return;
  }

  // ── Промокод (первый шаг после корзины) ───────────────────
  if (sess.step === DEL_STEP.PROMO) {
    if (text === 'Отмена') { sess.step = DEL_STEP.BASKET; await showBasket(peerId, uid, sess, 2); return; }
    if (text !== 'Пропустить') {
      const promoResult = applyPromo(text, 'delivery', sess.data.basket);
      if (!promoResult.ok) {
        await sendMessage(peerId, promoResult.msg + '\nВведите другой промокод или нажмите «Пропустить»:', { keyboard: msgKb([[{ label: 'Пропустить', color: 'secondary' }], [{ label: 'Отмена', color: 'negative' }]]) }, 2);
        return;
      }
      sess.data.promo = promoResult;
      if (promoResult.freeItem) sess.data.basket.push(promoResult.freeItem);
    }
    // Промокод применён/пропущен → вводим ник
    sess.step = DEL_STEP.CHECKOUT_NICK;
    const promoLine = sess.data.promo ? `Промокод: ${sess.data.promo.msg}\n\n` : '';
    await sendMessage(peerId, `${promoLine}Введите ваш никнейм в игре:`, { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
    return;
  }

  if (sess.step === DEL_STEP.CHECKOUT_CONF) {
    if (text === 'Изменить') {
      // Сбрасываем всё и возвращаем к промокоду
      sess.data.promo = null;
      sess.step = DEL_STEP.PROMO;
      await sendMessage(peerId, buildBasketText(sess.data.basket) + '\n\nЕсть промокод? Введите его или нажмите «Пропустить»:', { keyboard: msgKb([[{ label: 'Пропустить', color: 'secondary' }], [{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    ...
    if (text === 'Верно') {
      ...
      const rawTotal = basketTotal(sess.data.basket);
      let finalTotal = rawTotal - (promoResult?.discount || 0);
      if (promoResult?.promo?.type === 'free_delivery') {
        // бесплатная доставка = клиент платит себестоимость (нет дохода и зп курьеру с этого)
      ...
      await sendMessage(peerId, `Заказ оформлен!${promoText}\n\nОжидайте принятия заказа. Вы получите уведомление, когда курьер примет заказ.`,
        { keyboard: msgKb([[{ label: 'Статус заказа' }], [{ label: 'Отменить заказ', color: 'negative' }]]) }, 2);
      return;
    }
    return;
  }
  ...
  if (sess.step === DEL_STEP.WAITING || sess.step === DEL_STEP.ACTIVE) {
    ...
    if (text === 'Ссылка на курьера') {
      ...
      // Сначала спрашиваем у курьера согласие, потом отправляем ссылку клиенту
      if (!sess.data.courierLinkRequested) {
        sess.data.courierLinkRequested = true;
        // Уведомляем курьера и просим согласие
        await sendMessage(order.courierId,
          `Клиент ${order.nick} просит вашу ссылку для связи.\nОтветьте «Да» или «Нет»:`,
          { keyboard: msgKb([[{ label: 'Да — отправить ссылку', color: 'positive' }], [{ label: 'Нет — отклонить', color: 'negative' }]]) }, 1);
        await sendMessage(peerId, 'Запрос отправлен курьеру. Ожидайте ответа.', {}, 2);
      } else {
        await sendMessage(peerId, 'Запрос уже отправлен курьеру. Ожидайте ответа.', {}, 2);
      }
      return;
    }
    if (text === 'Да — отправить ссылку' || text === 'Нет — отклонить') {
      // Эти ответы обрабатываются в handleGroup1DM — здесь игнорируем
      return;
    }
    if (text === 'Отменить заказ') {
      const order = storage.activeOrders.get(sess.data.orderId);
      if (!order) { await sendMessage(peerId, 'Заказ не найден или уже завершён.', {}, 2); return; }
      // Нельзя отменить если курьер уже едет к клиенту или прибыл
      if (order.status === 'delivering' || order.status === 'arrived') {
        await sendMessage(peerId, 'Отменить нельзя — курьер уже в пути. Свяжитесь с курьером через ссылку.', {}, 2);
        return;
      }
      const cancelOrderId = sess.data.orderId;
      // Если курьер уже принял — уведомляем курьера об отмене
      if (order.status === 'accepted' && order.courierId) {
        await sendMessage(order.courierId,
          `Клиент отменил заказ ${getOrderNumDisplay(order, cancelOrderId)}. Заказ снят.`,
          { keyboard: msgKb([[{ label: 'Главное меню', color: 'secondary' }]]) }, 1);
        // Убираем purchase screen — сбрасываем staffSession курьера
        storage.staffSessions.delete(order.courierId);
      }
      // Удаляем из памяти
      storage.activeOrders.delete(cancelOrderId);
      // Обновляем файл orders.json
      const cancelOrds = readJSON(ORDERS_FILE, { delivery: [], taxi: [] });
      cancelOrds.delivery = cancelOrds.delivery.filter(o => o.id !== cancelOrderId);
      writeJSON(ORDERS_FILE, cancelOrds);
      // Обновляем сообщение в диспетчерской
      const cancelIds = storage.orderMsgIds.get(cancelOrderId);
      if (cancelIds) {
        await editMessage(cancelIds.chatId, cancelIds.dispatchMsgId,
          `Заказ ${getOrderNumDisplay(order, cancelOrderId)} — ОТМЕНЁН клиентом`, {}, 1);
        storage.orderMsgIds.delete(cancelOrderId);
      }
      sess.step = DEL_STEP.MAIN;
      sess.data = {};
      await sendMessage(peerId, 'Заказ отменён.',
...
function getOnlineCouriers(type) {
  ...
  storage.online.forEach((info, uid) => {
    ...
    if (!profile) return;
    // groups определяет организацию ��отруд��ика: ['delivery'], ['taxi'], или обе
...
async function handleGroup1DM(event) {
  ...
  if (step === 'courier_accept_eta') {
    ...
    // Обновляем сообщение в диспетчерской — указываем кто принял и статус
    const acceptMsgIds = storage.orderMsgIds.get(orderId);
    if (acceptMsgIds) {
      await editMessage(acceptMsgIds.chatId, acceptMsgIds.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — обрабатывается: ${order.courierNick}\nВремя ожидания: ${order.eta}`,
        { keyboard: kb([[{ label: `Принято: ${order.courierNick}`, color: 'positive', payload: {} }]]) }, 1);
    }
    ...
    await sendMessage(clientPeer,
      ...
      { keyboard: msgKb([[{ label: 'Статус заказа' }, { label: 'Ссылка на курьера' }], [{ label: 'Отменить заказ', color: 'negative' }]]) }, 2);
  ...
  if (text.startsWith('Отменить#')) {
    ...
    // Обновляем файл orders.json
    const cancelOrds = readJSON(ORDERS_FILE, { delivery: [], taxi: [] });
    const cancelList = order.type === 'taxi' ? cancelOrds.taxi : cancelOrds.delivery;
    const cancelIdx = cancelList.findIndex(o => o.id === orderId);
    if (cancelIdx !== -1) { cancelList[cancelIdx].status = 'cancelled'; writeJSON(ORDERS_FILE, cancelOrds); }
    // Обновляем сообщение в диспетчерской
    const cancelIds = storage.orderMsgIds.get(orderId);
    if (cancelIds) {
      await editMessage(cancelIds.chatId, cancelIds.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — ОТМЕНЁН курьером ${order.courierNick || ''}`, {}, 1);
      storage.orderMsgIds.delete(orderId);
    }
    // Сбрасываем сессию клиента
    for (const [cUid, cSess] of storage.clientSessions.entries()) {
      if (cSess.data && cSess.data.orderId === orderId) {
        cSess.step = DEL_STEP.MAIN;
        cSess.data = {};
      }
    }
    await sendMessage(order.clientId, 'Курьер отменил ваш заказ. Приносим извинения. Пожалуйста, оформите заказ повторно.',
      { keyboard: msgKb([[{ label: 'Заказать', color: 'positive' }, { label: 'Главное меню', color: 'secondary' }]]) }, gKey);
    storage.staffSessions.delete(uid);
    await sendMessage(peerId, `Заказ ${getOrderNumDisplay(order, orderId)} отменён.`, {}, 1);
    return;
  }

  // ── Courier: link consent (Да/Нет на запрос клиента) ─────
  if (text === 'Да — отправить ссылку' || text === 'Нет — отклонить') {
    let consentOrder = null;
    for (const [, o] of storage.activeOrders.entries()) {
      if (String(o.courierId) === String(uid)) { consentOrder = o; break; }
    }
    ...
    if (text === 'Да — отправить ссылку') {
      ...
      await sendMessage(clientId, 'Курьер отклонил запрос на отправку ссылки.', {}, 2);
    }
    // Восстанавливаем активные кнопки курьера в зависимости от состояния заказа
    await restoreCourierOrderButtons(uid, peerId, consentOrder);
    return;
  }

  // ── Courier: запросить ссылку на клиента ──────────────────
  if (text === 'Ссылка на клиента') {
    let courierOrder = null;
    for (const [, o] of storage.activeOrders.entries()) {
      if (String(o.courierId) === String(uid)) { courierOrder = o; break; }
    }
  ...
  if (step === STAFF_STEP.REG_BANK) {
    ...
    storage.staffSessions.set(uid, sess);
    await sendMessage(peerId,
      `Пр��филь создан!\nНик: ${sess.data.nick}\nБанк. счёт: ${sess.data.bank}\n\nТеперь до��а��ьте транспортное средство, чтобы принимать заказы.`,
      {
        keyboard: msgKb([
  ...
  if (text === 'Мой профиль' || text === 'проф��ль') {
    ...
    await sendMessage(peerId, text2, {
      keyboard: msgKb([
        [{ label: 'Ав��опарк', color: 'secondary' }, { label: 'Изменить ник', color: 'secondary' }, { label: 'Изменить счёт', color: 'secondary' }],
        [{ label: 'Главное меню', color: 'secondary' }],
      ])
    }, 1);
    return;
...
async function showGroup1MainMenu(uid, peerId, profile, isSs, isRs, role) {
  // Проверяем есть ли активный заказ у курьера
  let hasActiveOrder = false;
  for (const [, o] of storage.activeOrders.entries()) {
    if (String(o.courierId) === String(uid) && ['accepted', 'delivering', 'arrived'].includes(o.status)) {
      hasActiveOrder = true;
      break;
    }
  }
  ...
  // Если у курьера активный заказ — показываем кнопку «Ссылка на клиента»
  if (hasActiveOrder) {
    rows.push([{ label: 'Ссылка на клиента', color: 'primary' }]);
  }
  ...
  const activeOrderNote = hasActiveOrder ? '\nУ вас есть активный заказ.' : '';

  await sendMessage(peerId,
    `Привет, ${profile.nick}!\nРоль: ${roleLabel}${statusLine}${activeOrderNote}\n\nВыберите раздел:`,
...
async function handleAdminCatalogueSession(uid, peerId, text, event) {
  ...
  if (step === 'admin_item_name') {
    ...
    await sendMessage(peerId, 'Введите стоимость товара (дл�� клиентов, в рублях):', {}, 1);
    return true;
  }
...
if (false) { // TAXI_DISABLED_START
  ...
  async function sendCategorySelect(peerId, groupKey, label) {
    ...
    if (!tp.categories.length) {
      await sendMessage(peerId,
        'Точки маршрута ещё не добавлены. Обратитесь к администратору.',
  ...
  async function handleTaxiDM(event) {
    ...
    if (sess.step === TAXI_STEP.ORDER_PROMO) {
      ...
      await sendMessage(peerId,
        ...
        { keyboard: msgKb([[{ label: 'Наличными' }], [{ label: 'Счёт теле��она' }], [{ label: 'Банковский счёт' }], [{ label: 'Отмена', color: 'negative' }]]) }, 3);
      return;
    }
...
async function handleCallback(event, groupKey) {
  ...
  // Cancel order (courier cancels from purchase/deliver screen)
  if (action === 'cancel_order') {
    ...
    if (cancelIdx !== -1) { cancelList[cancelIdx].status = 'cancelled'; writeJSON(ORDERS_FILE, cancelOrds); }
    // Обновляем сообщение в диспетчерской
    const cancelDispIds = storage.orderMsgIds.get(orderId);
    if (cancelDispIds) {
      await editMessage(cancelDispIds.chatId, cancelDispIds.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — ОТМЕНЁН курьером ${order.courierNick || ''}`, {}, 1);
      storage.orderMsgIds.delete(orderId);
    }
    // Сбрасываем сессию клиента и уведомляем его
    for (const [, cSess] of storage.clientSessions.entries()) {
      if (cSess.data && cSess.data.orderId === orderId) {
        cSess.step = DEL_STEP.MAIN;
        cSess.data = {};
      }
    ...
    await sendMessage(order.clientId,
      `Курьер отменил заказ ${getOrderNumDisplay(order, orderId)}. Приносим извинения. Пожалуйста, оформите заказ повторно.`,
      { keyboard: msgKb([[{ label: 'Каталог', color: 'secondary' }, { label: 'Заказать', color: 'positive' }], [{ label: 'Частые вопросы', color: 'secondary' }]]) }, gKey);
    storage.staffSessions.delete(uid);
    await sendMessage(uid, `Заказ ${getOrderNumDisplay(order, orderId)} отменён.`,
      { keyboard: msgKb([[{ label: 'Главное меню', color: 'secondary' }]]) }, 1);
    return;
  }
  ...
  if (action === 'toggle_buy') {
    ...
    buttons.push([{ label: 'Отменить заказ', color: 'negative', payload: { action: 'cancel_order', orderId } }]);
  ...
  if (action === 'cooking_done') {
    ...
    const order = storage.activeOrders.get(orderId) || storage.activeTaxi.get(orderId);
    if (!order) return;
    order.status = 'accepted'; // статус: принят и готовится
    // Update dispatch message
    const cookIds = storage.orderMsgIds.get(orderId);
    if (cookIds) {
  ...
  if (action === 'start_deliver') {
    ...
    const deliverIds = storage.orderMsgIds.get(orderId);
    if (deliverIds) {
    ...
    await sendMessage(uid,
      `Заказ ${getOrderNumDisplay(order, orderId)}:\nКлиент: ${order.nick}\nАдрес: ${order.address || ((order.from?.name || '') + ' → ' + (order.to?.name || '')) || '—'}\nСумма: ${order.total || order.finalPrice}р.\n\nНажмите когда прибудете к клиенту:`,
      { keyboard: kb([[{ label: 'Прибыл к клиенту', color: 'positive', payload: { action: 'courier_arrived', orderId } }], [{ label: 'Отменить заказ', color: 'negative', payload: { action: 'cancel_order', orderId } }]]) }, 1);
    // Notify client — теперь нельзя отменить
    await sendMessage(order.clientId,
      `Заказ готов! Курьер ${order.courierNick} едет к вам.`,
  ...
  if (action === 'courier_arrived') {
    ...
    // Update dispatch
    const arrivedIds = storage.orderMsgIds.get(orderId);
    if (arrivedIds) {
  ...
  if (action === 'finish_order') {
    ...
    // Обновляем сообщение в диспетчерской — заказ выполнен
    const finishIds = storage.orderMsgIds.get(orderId);
    if (finishIds) {
      await editMessage(finishIds.chatId, finishIds.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — ВЫПОЛНЕН курьером ${order.courierNick || ''}`, {}, 1);
      storage.orderMsgIds.delete(orderId);
    }
    ...
    await sendMessage(order.clientId,
      ...
      { keyboard: msgKb([[{ label: 'Каталог', color: 'secondary' }, { label: 'Заказать', color: 'positive' }], [{ label: 'Трудоустройство', color: 'secondary' }, { label: 'Частые вопросы', color: 'secondary' }]]) },
      order.type === 'taxi' ? 3 : 2);
...
async function main() {
  ...
  console.log('[Bot] Инициализация планир��вщика отчётов...');
