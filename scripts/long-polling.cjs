...
const TAXI_PICKS_FILE = path.join(DATA_DIR, 'taxi_picks.json');
...
const DEL_STEP = {
  ...
  PROMO: 'del_promo',
  BIG_ORDER_DATE: 'del_big_date',
  BIG_ORDER_TIME: 'del_big_time',
  WAITING: 'del_waiting',
  ACTIVE: 'del_active',
};
...
// ─────────────────────────── SLOT COUNT HELPERS ───────────────
/**
 * Считает суммарное количество «простых слотов» в корзине.
 * Простой товар = 1 слот за единицу.
 * Сложный товар (с subItems) = сумма qty всех subItems × qty самого товара.
 */
function countBasketSlots(basket) {
  const cat = loadCatalogue();
  let total = 0;
  for (const item of basket) {
    const fullItem = cat.items.find(it => it.id === item.id) ||
      cat.sets.find(s => 'set_' + s.id === item.id);
    if (fullItem && fullItem.subItems && fullItem.subItems.length > 0) {
      const subTotal = fullItem.subItems.reduce((s, si) => s + (si.qty || 1), 0);
...
/**
 * Минимальная дата доставки для крупного заказа.
 * slots > 50 → +1 час за каждые 50 слотов сверх базы, минимум 3 часа.
 * Возвращает Date объект.
 */
function bigOrderMinDate(slots) {
  const extraHours = Math.floor((slots - 51) / 50);
  const minHours = 3 + extraHours;
  return new Date(Date.now() + minHours * 3600 * 1000);
}

/**
 * Форматирует минимальную дату в читаемый вид (МСК).
 */
function formatMinDate(slots) {
  const d = bigOrderMinDate(slots);
  const msk = new Date(d.toLocaleString('en-US', { timeZone: 'Europe/Moscow' }));
  return `${pad(msk.getDate())}.${pad(msk.getMonth() + 1)}.${msk.getFullYear()} ${pad(msk.getHours())}:${pad(msk.getMinutes())}`;
}
...
async function handleDeliveryDM(event) {
  ...
  if (sess.step === DEL_STEP.CHECKOUT_ADDR) {
    ...
    await sendMessage(peerId, summary, { keyboard: msgKb([[{ label: 'Верно', color: 'positive' }, { label: 'Изменить адрес', color: 'secondary' }, { label: 'Изменить ник', color: 'secondary' }], [{ label: 'Изменить корзину', color: 'secondary' }]]) }, 2);
    return;
  }

  if (sess.step === DEL_STEP.CHECKOUT_CONF) {
    if (text === 'Измен��ть ник') {
      sess.step = DEL_STEP.CHECKOUT_NICK;
      await sendMessage(peerId, 'Введите новый никнейм:', { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    if (text === 'Изменить адрес') {
      sess.step = DEL_STEP.CHECKOUT_ADDR;
      await sendMessage(peerId, 'Введите новое место доставки:', { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    if (text === 'Изменить корзину') {
      // Go back to basket view so user can add/remove items
      sess.step = DEL_STEP.BASKET;
      await showBasket(peerId, uid, sess, 2);
      return;
    }
  ...
  if (sess.step === DEL_STEP.PROMO) {
    ...
    // Проверяем количество слотов (> 50 = крупный заказ)
    const slots = countBasketSlots(sess.data.basket);
    if (slots > 50) {
      // Крупный заказ — запрашиваем дату
      sess.data.promoResult = promoResult;
      sess.data.slots = slots;
      sess.step = DEL_STEP.BIG_ORDER_DATE;
      const minDate = formatMinDate(slots);
      await sendMessage(peerId,
        `Ваш заказ очень большой (${slots} слотов).\n\nПожалуйста, выберите дату доставки.\nМинимально доступная дата: ${minDate} (МСК)\n\nВведите дату в формате ДД.ММ.ГГГГ:`,
        { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }

    await createAndDispatchOrder(peerId, uid, sess, promoResult, slots);
    return;
  }

  // ── Крупный заказ: выбор даты ─────────────────────────────
  if (sess.step === DEL_STEP.BIG_ORDER_DATE) {
    if (text === 'Отмена') { sess.step = DEL_STEP.BASKET; await showBasket(peerId, uid, sess, 2); return; }
    // Валидируем формат ДД.ММ.ГГГГ
    const dateMatch = text.match(/^(\d{2})\.(\d{2})\.(\d{4})$/);
    if (!dateMatch) {
      await sendMessage(peerId, 'Неверный формат. Введите дату в формате ДД.ММ.ГГГГ:', { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    const [, dd, mm, yyyy] = dateMatch;
    const inputDate = new Date(`${yyyy}-${mm}-${dd}T00:00:00+03:00`);
    const minDate = bigOrderMinDate(sess.data.slots);
    if (isNaN(inputDate.getTime()) || inputDate < new Date(minDate.getFullYear(), minDate.getMonth(), minDate.getDate())) {
      await sendMessage(peerId, `Дата слишком ранняя. Минимальная дата: ${formatMinDate(sess.data.slots)}\nВведите дату повторно:`, { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    sess.data.bigOrderDate = text;
    sess.step = DEL_STEP.BIG_ORDER_TIME;
    await sendMessage(peerId, `Дата принята: ${text}\n\nВведите желаемое время в формате ЧЧ:ММ (МСК):`, { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
    return;
  }

  if (sess.step === DEL_STEP.BIG_ORDER_TIME) {
    if (text === 'Отмена') { sess.step = DEL_STEP.BIG_ORDER_DATE; const minDate = formatMinDate(sess.data.slots); await sendMessage(peerId, `Введите дату (мин. ${minDate}):`, { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2); return; }
    const timeMatch = text.match(/^(\d{1,2}):(\d{2})$/);
    if (!timeMatch) {
      await sendMessage(peerId, 'Неверный формат. Введите время ЧЧ:ММ:', { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    const [, hh, mm2] = timeMatch;
    if (parseInt(hh) > 23 || parseInt(mm2) > 59) {
      await sendMessage(peerId, 'Неверное время. Введите время ЧЧ:ММ:', { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
      return;
    }
    sess.data.bigOrderTime = `${String(hh).padStart(2, '0')}:${mm2}`;
    // Проверяем что если дата совпадает с минимальной — время тоже подходит
    const minDt = bigOrderMinDate(sess.data.slots);
    const mskMinStr = formatMinDate(sess.data.slots); // ДД.ММ.ГГГГ ЧЧ:ММ
    const minDateStr = mskMinStr.split(' ')[0]; // ДД.ММ.ГГГГ
    if (sess.data.bigOrderDate === minDateStr) {
      const minHHMM = mskMinStr.split(' ')[1];
      if (sess.data.bigOrderTime < minHHMM) {
        await sendMessage(peerId, `В эту дату минимальное время: ${minHHMM} (МСК). Введите время снова:`, { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, 2);
        return;
      }
    }
    await createAndDispatchOrder(peerId, uid, sess, sess.data.promoResult, sess.data.slots);
    return;
  }
  ...
  if (sess.step === DEL_STEP.WAITING || sess.step === DEL_STEP.ACTIVE) {
    ...
    if (text === 'Отменить заказ') {
      ...
      if (order.status !== 'pending') {
        await sendMessage(peerId, 'Отме��ить можно только заказ, который ещё не принят курьером.', {}, 2);
        return;
      }
      ...
      if (clientCancelIds) {
        await editMessage(clientCancelIds.chatId, clientCancelIds.dispatchMsgId,
          ...
          { keyboard: kb([[{ label: 'Отменён клиентом', color: 'negative', payload: {} }]]) }, 1);
      }
...
/** Создаёт заказ, сохраняет и отправляет в диспетчерскую / SS */
async function createAndDispatchOrder(peerId, uid, sess, promoResult, slots) {
  const orderId = genId();
  const orderNum = incrementOrderNumber('delivery');
  const costTotal = sess.data.basket.reduce((sum, it) => sum + (it.cost || 0) * it.qty, 0);
  const isBig = (slots || 0) > 50;
  const order = {
    id: orderId,
    num: orderNum,
    type: 'delivery',
  ...
  const schedText = order.scheduledDate ? `\nЖелаемая дата: ${order.scheduledDate} ${order.scheduledTime}` : '';

  if (isBig) {
    // Крупный заказ: бот далее не ведёт, SS работает вручную
    sess.step = DEL_STEP.MAIN;
    sess.data = {};
    await sendMessage(peerId,
      `Заказ оформлен!${promoText}${schedText}\n\nВаш заказ крупный и передан старшему составу для ручной обработки. С вами свяжутся в ближайшее время.`,
      { keyboard: msgKb([[{ label: 'Главное меню', color: 'secondary' }]]) }, 2);
    return;
  }
...
async function sendOrderToDispatch(order) {
  ...
  const schedLine = order.scheduledDate ? `\nЖелаемая дата: ${order.scheduledDate} ${order.scheduledTime}` : '';
  const bigLabel = order.isBig ? ' [КРУПНЫЙ ЗАКАЗ]' : '';

  // @all в начале сообщения (упоминание всех участников чата)
  const atAll = '@all\n';

  const promoLine = order.promo ? `\nПромокод: ${order.promo}${order.promoDiscount ? ` — скидка ${order.promoDiscount}р.` : ''}` : '';
  const text = `${atAll}Новый заказ ${orderNumDisplay}${bigLabel}\n\nНикнейм: ${order.nick}\nМесто: ${order.address}${schedLine}\nЗаказ:\n${lines.join('\n')}\nИтог: ${total}р.${costLine}${promoLine}`;

  // Крупный заказ идёт в беседу Старшего Состава (без кнопок — ведётся вручную),
  // обычный — в Диспетчерскую с кнопкой «Принять заказ»
  const isDelivery = order.type !== 'taxi';
  let chatId;
  if (order.isBig) {
    chatId = isDelivery ? CHATS.ss : CHATS.taxiSs;
  } else {
  ...
  console.log(`[Bot] sendOrderToDispatch: order=${order.id}, type=${order.type}, isBig=${order.isBig}, chatId=${chatId}`);

  if (!chatId || chatId === 0) {
    console.error(`[Bot] sendOrderToDispatch ERROR: chatId не настроен для типа ${order.type}, isBig=${order.isBig}`);
    return;
  }
  ...
  // Auto-check after 3 minutes if not accepted (только для обычных заказов)
  if (!order.isBig) {
...
async function handleCourierAcceptOrder(event, orderId) {
  ...
  if (order.status !== 'pending' && order.status !== 'processing') { await sendMessage(uid, 'Заказ уже принят.', {}, 1); return; }
  ...
  // Mark as accepted immediately to block race (other couriers clicking the same button)
  order.status = 'processing';

  // Update dispatch message: replace "Принять заказ" with status label
...
async function buildPurchaseScreen(uid, order) {
  ...
  buttons.push([{ label: 'Ссылка на клиента', color: 'secondary', payload: { action: 'get_client_link', orderId: order.id } }]);
...
async function handleGroup1DM(event) {
  ...
  if (text.startsWith('Отменить#')) {
    ...
    if (legacyCancelIds) {
      // Restore with "Принять заказ" so another courier can take it
      await editMessage(legacyCancelIds.chatId, legacyCancelIds.dispatchMsgId,
        ...
        { keyboard: kb([[{ label: 'Принять заказ', color: 'positive', payload: { action: 'accept_order', orderId: order.id } }]]) }, 1);
      order.status = 'pending';
      order.courierId = null;
      order.courierNick = null;
      order.eta = null;
      storage.activeOrders.set(orderId, order);
    }
    const clientId = parseInt(order.clientId);
    storage.clientSessions.set(clientId, { step: DEL_STEP.MAIN, data: {} });
    await sendMessage(clientId, 'Курьер отменил ваш заказ. Приносим извинения. Пожалуйста, оформите заказ повторно.',
  ...
  // ── Courier: request client VK link (no approval needed) ─────
  if (text === 'Ссылка на клиента') {
    // Find the active order belonging to this courier
    const courierOrder = [...storage.activeOrders.values(), ...storage.activeTaxi.values()]
      .find(o => o.courierId === uid && !['done', 'cancelled'].includes(o.status));
    if (!courierOrder) {
      await sendMessage(peerId, 'У вас нет активного заказа.', {}, 1);
    } else {
  ...
  if (text === 'Мой профиль' || text === 'проф��ль') {
    ...
    await sendMessage(peerId, text2, {
      keyboard: msgKb([
  ...
  if (isRs && text === 'Управле��ие промокодами') {
    const promos = readJSON(PROMOS_FILE, { delivery: [], taxi: [] });
...
async function showVehicleMenu(uid, peerId, profile) {
  ...
  await sendMessage(peerId, menuText, {
    keyboard: msgKb([
      [{ label: '��обавить личное авто', color: 'positive' }, { label: 'Взять авто организации', color: 'primary' }],
...
async function handleAdminCommand(uid, peerId, text, role) {
  ...
  if (text === 'Управление каталогом') {
    ...
    await sendMessage(peerId,
      ...
      {
        keyboard: msgKb([
          [{ label: 'Добавить категорию' }, { label: 'Добав��ть товар' }],
...
async function handleAdminCatalogueSession(uid, peerId, text, event) {
  ...
  if (step === 'admin_item_cost') {
    ...
    await sendMessage(peerId, 'Температур���� / тип (например «80°с») или «Пропустить»:', { keyboard: msgKb([[{ label: 'Пропустить' }]]) }, 1);
    return true;
  }
...
async function handleAdminTaxiPoints(uid, peerId, text, event) {
  ...
  if (step === 'tp_add_point_price') {
    ...
    sess.step = 'tp_add_point_coords';
    storage.adminSessions.set(uid, sess);
    await sendMessage(peerId,
      `Точка: «${sess.data.tpPointName}» (${price}р.)\n\nВведите координаты точки на карте в формате «X,Y» (целые числа из map-editor.html).\nКоординаты нужны для отображени�� точки на карте и точного расчёта стоимости по прямой.\n\nИли нажмите «Пропустить» (расчёт будет по базовой цене):`,
      { keyboard: msgKb([[{ label: 'Пропустить', color: 'secondary' }], [{ label: 'Отмена', color: 'negative' }]]) }, 1);
    return true;
  }
  if (step === 'tp_add_point_coords') {
    if (text === 'Отмена') { sess.step = null; storage.adminSessions.set(uid, sess); await sendMessage(peerId, 'Отменено.', {}, 1); return true; }
    let x, y;
    if (text !== 'Пропустить') {
      ...
      if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
        ...
        await sendMessage(peerId, 'Неверный формат. Введите «X,Y» (например: 450,320) или нажмите «Пропустить»:', { keyboard: msgKb([[{ label: 'Пропустить', color: 'secondary' }]]) }, 1);
        return true;
      }
    }
    const tp = readJSON(TAXI_POINTS_FILE, { categories: [], points: [] });
    if (!Array.isArray(tp.points)) tp.points = [];
    const newPoint = { id: genId(), categoryId: sess.data.tpCatId, name: sess.data.tpPointName, basePrice: sess.data.tpPointPrice };
    if (x !== undefined) { newPoint.x = x; newPoint.y = y; }
    tp.points.push(newPoint);
    writeJSON(TAXI_POINTS_FILE, tp);
    sess.step = null; storage.adminSessions.set(uid, sess);
    const coordInfo = x !== undefined ? ` | коорд: ${x},${y}` : ' | без коорд.';
    await sendMessage(peerId,
      `Точка «${sess.data.tpPointName}» (${sess.data.tpPointPrice}р.${coordInfo}) добавлена.`,
      { keyboard: msgKb([[{ label: 'Управление точками такси', color: 'primary' }, { label: 'Добавить точку', color: 'positive' }], [{ label: 'Главное меню', color: 'secondary' }]]) }, 1);
    return true;
  }
...
if (false) { // TAXI_DISABLED_START
  const TAXI_STEP = {
    ...
    ORDER_FROM_CAT: 'taxi_from_cat',     // select "from" category
    ORDER_FROM_POINT: 'taxi_from_point',   // select "from" point
    ORDER_TO_CAT: 'taxi_to_cat',       // select "to" category
    ORDER_TO_POINT: 'taxi_to_point',     // select "to" point
  ...
  // ── Helpers ────────────────────────────────────────────────────
  function readTaxiPicks() {
    return readJSON(TAXI_PICKS_FILE, {});
  }
  ...
  /** Return taxi points data { categories, points } */
  function readTaxiPoints() {
    const tp = readJSON(TAXI_POINTS_FILE, { categories: [], points: [] });
    if (!Array.isArray(tp.categories)) tp.categories = [];
    if (!Array.isArray(tp.points)) tp.points = [];
    return tp;
  ...
  /** Build a category-select keyboard for "from" or "to" step */
  async function sendCategorySelect(peerId, groupKey, label) {
    const tp = readTaxiPoints();
    if (!tp.categories.length) {
      await sendMessage(peerId,
        'Точки маршру��а ещё не добавлены. Обратитесь к администратору.',
        { keyboard: msgKb([[{ label: 'Отмена', color: 'negative' }]]) }, groupKey);
      return false;
    }
    const rows = tp.categories.map(c => {
      const cnt = tp.points.filter(p => p.categoryId === c.id).length;
      return [{ label: `${c.name} (${cnt})` }];
    });
    rows.push([{ label: 'Отмена', color: 'negative' }]);
    await sendMessage(peerId,
      `${label}\nВыберите категорию:`,
  ...
  /** Build a points-select keyboard for chosen category */
  async function sendPointSelect(peerId, groupKey, catName, label, excludeId = null) {
    const tp = readTaxiPoints();
    const cat = tp.categories.find(c => c.name === catName);
    if (!cat) return false;
    const pts = tp.points.filter(p => p.categoryId === cat.id && p.id !== excludeId);
    if (!pts.length) {
      await sendMessage(peerId, 'В этой категории нет доступных точек.', {}, groupKey);
      return false;
    }
    const rows = pts.map(p => [{ label: p.name }]);
    rows.push([{ label: '← Назад', color: 'secondary' }, { label: 'Отмена', color: 'negative' }]);
    await sendMessage(peerId,
      `${label}\nКатегория: ${catName}\nВыберите точку:`,
      { keyboard: msgKb(rows) }, groupKey);
    return true;
  ...
  /**
   * Send map-picker link to client.
   * The link opens /taxi-order?token=TOKEN&step=from
   * The page lets them pick "from" then "to", posting to /api/taxi-pick.
   * Bot then polls /api/taxi-pick?token=TOKEN every 2 s until both points are set.
   */
  async function sendMapPickerLink(peerId, token, groupKey) {
    ...
    await sendMessage(peerId,
      `Выберите точки маршрута на карте.\n\nНажмите кнопку ниже — откроется ка��та. Сначала выберите откуда, затем куда. После выбора вернитесь сюда, бот продолжит автоматически.\n\nСсылка на карту:\n${url}`,
      {
        keyboard: msgKb([
  ...
  /**
   * Poll loop: every 2 s check taxi_picks.json for the given token.
   * When both from+to are present, continue the order flow.
   * Stops after 10 min or if session is cancelled.
   */
  function startMapPollLoop(uid, peerId, token, groupKey) {
    const MAX_TRIES = 300; // 10 min × 2s
    let tries = 0;

    const interval = setInterval(async () => {
      tries++;
      const sess = storage.clientSessions.get('taxi_' + uid);
      if (!sess || sess.step !== TAXI_STEP.ORDER_MAP_WAIT || sess.data.mapToken !== token) {
      ...
      if (!entry) {
        if (tries >= MAX_TRIES) {
          ...
          await sendMessage(peerId, 'Время выбора маршрута истекло. Попробуйте ещё раз.', {
            keyboard: msgKb([[{ label: 'Заказать такси', color: 'positive' }], [{ label: 'Главное меню', color: 'secondary' }]]),
          }, groupKey);
        }
        return;
      }

      // Check both steps present
      if (!entry.from || !entry.to) return;

      clearInterval(interval);
      clearTaxiPick(token);

      sess.data.from = entry.from;
      sess.data.to = entry.to;
  ...
  async function handleTaxiDM(event) {
    ...
    if (text === 'начать' || text === '/start') {
      sess.step = TAXI_STEP.MAIN; sess.data = {};
      storage.clientSessions.set('taxi_' + uid, sess);
      await sendMessage(peerId, 'Сервис такси. Выберите раздел:', { keyboard: mainKb }, 3);
      return;
    }
    if (text === 'Главное меню') {
      sess.step = TAXI_STEP.MAIN; sess.data = {};
      storage.clientSessions.set('taxi_' + uid, sess);
      await sendMessage(peerId, 'Главное меню:', { keyboard: mainKb }, 3);
      return;
    }
    if (text === 'Трудоустройство') {
    ...
    if (sess.step === TAXI_STEP.ORDER_TO_POINT) {
      ...
      storage.clientSessions.set('taxi_' + uid, sess);
      await sendMessage(peerId,
        `Маршрут выбран:\nОткуда: ${sess.data.from.name}\nКуда: ${sess.data.to.name}\nПримерная стоимость: ${price}р.\n\nЕсть промокод? Введите или нажмит�� «Пропустить»:`,
...
async function handleChatCommand(event, groupKey) {
  ...
  if (cmd === '!закреп') {
    ...
    try {
      ...
      await sendMessage(peerId, 'Сообщен��е закреплено', {}, groupKey);
  ...
  if (cmd === '!разбан') {
    ...
    await sendMessage(peerId, ok ? `Пользователь ${targetId} разбанен` : 'Пользовател�� не в б��не', {}, groupKey);
    return true;
  }
...
async function handleCallback(event, groupKey) {
  ...
  if (action === 'accept_order') {
    ...
    if (order.status !== 'pending') {
      // Already accepted — do nothing, dispatch message already shows status
      return;
    }
  ...
  if (action === 'toggle_buy') {
    ...
    if (allBought) {
      // All items bought — update dispatch, show "Еду к клиенту" button
      order.status = 'ready';
      const ids = storage.orderMsgIds.get(orderId);
      if (ids) {
        await editMessage(ids.chatId, ids.dispatchMsgId,
          `Заказ ${getOrderNumDisplay(order, orderId)} — закуплено, курьер выезжает (курьер: ${order.courierNick})`,
          { keyboard: kb([[{ label: 'Статус: Закуплено', color: 'secondary', payload: {} }]]) }, 1);
      }
  ...
  // Courier: get client link directly (no approval)
  if (action === 'get_client_link') {
    const { orderId } = payload;
    const order = storage.activeOrders.get(orderId) || storage.activeTaxi.get(orderId);
    if (!order) { await sendMessage(uid, 'Заказ не найден.', {}, 1); return; }
    await sendMessage(uid, `Ссылка на клиента: vk.me/id${order.clientId}`, {}, 1);
    return;
  }
  ...
  if (action === 'courier_cancel_order') {
    const { orderId } = payload;
    const order = storage.activeOrders.get(orderId) || storage.activeTaxi.get(orderId);
    if (!order) { await sendMessage(uid, 'Заказ не найден или уже завершён.', {}, 1); return; }
    console.log(`[v0] courier_cancel_order: orderId=${orderId}, clientId=${order.clientId}, type=${order.type}`);
    const gKey = order.type === 'taxi' ? 3 : 2;
    storage.activeOrders.delete(orderId);
    storage.activeTaxi.delete(orderId);

    // Restore dispatch message with "Принять заказ" button so another courier can take it
    const cancelIds = storage.orderMsgIds.get(orderId);
    console.log(`[v0] courier_cancel_order: cancelIds=${JSON.stringify(cancelIds)}`);
    if (cancelIds) {
      await editMessage(cancelIds.chatId, cancelIds.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — ОТМЕНЁН курьером ${order.courierNick || ''}\n(доступен повторно)`,
        { keyboard: kb([[{ label: 'Принять заказ', color: 'positive', payload: { action: 'accept_order', orderId: order.id } }]]) }, 1);
      // Re-add to active orders as pending so it can be re-accepted
      order.status = 'pending';
      order.courierId = null;
      order.courierNick = null;
      order.eta = null;
    ...
    // Notify client
    const clientId = parseInt(order.clientId);
    console.log(`[v0] courier_cancel_order: sending to clientId=${clientId} with gKey=${gKey}`);
    await sendMessage(clientId, 'Курьер отменил ваш заказ. Приносим извинения. Пожалуйста, оформите заказ повторно.',
      { keyboard: msgKb([[{ label: 'Главное меню', color: 'secondary' }]]) }, gKey);

    const clientSess = storage.clientSessions.get(clientId) || storage.clientSessions.get(order.clientId);
  ...
  if (action === 'start_deliver') {
    ...
    const gKey = order.type === 'taxi' ? 3 : 2;
    // Update dispatch
    const ids3 = storage.orderMsgIds.get(orderId);
    if (ids3) {
      await editMessage(ids3.chatId, ids3.dispatchMsgId,
        `Заказ ${getOrderNumDisplay(order, orderId)} — курьер едет (курьер: ${order.courierNick})`,
        { keyboard: kb([[{ label: 'Статус: В пути', color: 'secondary', payload: {} }]]) }, 1);
    }
  ...
  if (action === 'finish_order') {
    ...
    const staff2 = readJSON(STAFF_FILE, {});
    if (order.courierId && staff2[String(order.courierId)]) {
      if (order.type === 'taxi') staff2[String(order.courierId)].stats.taxiOrders = (staff2[String(order.courierId)].stats.taxiOrders || 0) + 1;
      else staff2[String(order.courierId)].stats.deliveryOrders = (staff2[String(order.courierId)].stats.deliveryOrders || 0) + 1;
      writeJSON(STAFF_FILE, staff2);
    }

    const ords2 = readJSON(ORDERS_FILE, { delivery: [], taxi: [] });
    const list2 = order.type === 'taxi' ? ords2.taxi : ords2.delivery;
    const idx2 = list2.findIndex(o => o.id === orderId);
    if (idx2 !== -1) list2[idx2] = order;
    writeJSON(ORDERS_FILE, ords2);
    ...
    const reviewLink = `vk.com/wall-${G1_ID}?w=wall-${G1_ID}_1`;
    await sendMessage(order.clientId,
  ...
  // Allow courier link (legacy client-side flow — kept for backward compat)
  if (action === 'allow_link') {
    const { orderId } = payload;
    const order = storage.activeOrders.get(orderId) || storage.activeTaxi.get(orderId);
    if (!order) { await sendMessage(uid, 'Заказ не найден.', {}, 1); return; }
    const gKey2 = order.type === 'taxi' ? 3 : 2;
    await sendMessage(order.clientId, `Ссылка на переписку с курьером: vk.me/id${order.courierId}`, {}, gKey2);
  ...
  if (action === 'deny_link') {
    ...
    const order = storage.activeOrders.get(orderId) || storage.activeTaxi.get(orderId);
    if (!order) return;
    const gKey3 = order.type === 'taxi' ? 3 : 2;
    await sendMessage(order.clientId, 'Курьер отклонил запрос на ссылку.', {}, gKey3);
