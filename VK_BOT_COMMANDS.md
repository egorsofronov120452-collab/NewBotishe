...
**Использование**: 
```
!пост [доставка|такси] (ответом на сообщение)
```

**Описание**: 
Публикует сообщение на стену группы Доставки или Такси. Поддерживает все типы вложений:
...
- Если команда используется в чатах доставки (диспетчерская, СС, флудилка, учебный центр) — публикуется в группу Доставки
- Если команда используется в чатах такси — публикуется в группу Такси
- Можно явно указать группу: `!пост доставка` или `!пост такси`

**Примеры**:
```
!пост
```
Опубликует в группу, определённую автоматически по текущему чату

```
!пост доставка
```
Опубликует в группу Доставки (группа 2)

```
!пост такси
```
Опубликует в группу Такси (группа 3)

**Как использовать**:
1. Напишите сообщение с текстом и прикрепите фото/видео/документы
2. Ответьте на это сообщение командой `!пост` или `!пост [доставка|такси]`
3. Сообщение будет опубликовано в соответствующей группе со всеми вложениями
